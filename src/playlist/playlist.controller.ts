...
export class PlaylistController {
  ...
  @ApiOperation({
    ...
    description: `지정된 ID의 플레이리스트를 수정합니다.<br>
                  전달된 동영상 배열로 기존 플레이리스트를 완전히 덮어씁니다.<br>
                  동영상 배열이 비어 있으면 동영상은 모두 삭제되고 태그, 제목, 설명만 수정됩니다.`,
  })
  @ApiBody({
    ...
    schema: {
      type: 'object',
      properties: {
        title: { type: 'string', example: 'Updated Playlist Title' },
        description: { type: 'string', example: 'Updated description' },
        tags: { type: 'array', items: { type: 'string' }, example: ['Pop', 'K-Pop'] },
        videos: {
          type: 'array',
          items: {
            type: 'object',
